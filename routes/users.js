...
router.post("/deleteUserAccount", async (req, res, next) => {
  ...
  try {
    ...
    try {
      const delNotice = {
        to: "laflosd@gmail.com",
        from: process.env.AUTH_EMAIL,
        subject: "User Account Deletion Notice",
        text: "User account is being deleted from JejeSales Marketplace",
      ...
      console.error("Failed to send deletion notice email:", error.message);
    }

    // Save user profile details to DeletedUser
    ...
    // Find and copy active listings to DeletedListing collection (preserving data for 30 days)
    const userListings = await Listing.find({ userId });
    if (userListings && userListings.length > 0) {
      const deletedListingsData = userListings.map(listing => ({
        title: listing.title,
      ...
      // Use insertMany to store all listings, catching duplication errors
      try {
        await DeletedListing.insertMany(deletedListingsData);
      } catch (err) {
        console.error("Failed to archive listings to DeletedListing:", err.message);
      }
    }

    // Delete active listing documents
...
// DELETE A USER REMOVED (CONSOLIDATED AT TOP OF FILE)
